...
class LevelRuleComp:
    ...
    def _compile_item_rule(self, irule: lrb.ItemRule) -> RegionRule:  # noqa: C901
        match irule:
            ...
            case _:
                raise NotImplementedError(
                    f"{irule.source_path}: '{type(irule).__name__}' is not a supported item rule type"
                )
    ...
    def compile_rule_expr(self, rule_expr: lrb.RuleExpr) -> RegionRule:
        match rule_expr:
            ...
            case lrb.ItemRule():
                return self._compile_item_rule(rule_expr)
            case lrb.And():
            ...
            case lrb.Not():
                return rb.rrule_not(self.compile_rule_expr(rule_expr.item))
            case _:
                raise NotImplementedError(
