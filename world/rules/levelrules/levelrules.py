...
from .levelrulebase import LevelRule

LRULES: dict[str, LevelRule] = {}
def lrule(fn: LevelRule) -> LevelRule:
    _name = fn.__name__.removeprefix("lrule_")
    LRULES[_name] = fn
    return fn
...
def lrule_topgrade(wconf: WorldConfig) -> RegionRule:
    ...
    if (wconf.weapon_mode & (WeaponMode.PROGRESSIVE | WeaponMode.EX_SEPARATE)) > 0:
        _rule = rb.rrule_and(
            _rule,
            rb.rrule_or(lrule_any_super(wconf), lrule_weapon_ex(wconf))
        )
    return _rule
