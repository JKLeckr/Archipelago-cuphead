...
data_reg: dict[str, DataType] = {
    "levelruledefs": DataType.JSON,
    "levelrulepresets": DataType.JSON
}
...
def load_data(dataname: DataName) -> None:
    ...
    try:
        with data_files.joinpath(dataname).open("rb") as file:
            data = file.read()
        if data_reg[dataname] == DataType.STR:
            data = data.decode("utf-8")
        elif data_reg[dataname] == DataType.JSON:
...
def get_json_data(dataname: DataName) -> Any:
    _data = get_data(dataname)
    if data_reg[dataname] != DataType.JSON:
